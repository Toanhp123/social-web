...
import { ListFriendRequestsInput } from '@/modules/friends/domain/types/friend.type.js';
import { FriendRequestRepository } from '../../domain/repositories/friend-request.repository.interface.js';

@Injectable()
export class ListFriendRequestsService {
  constructor(
    @Inject(FRIEND_REQUEST_REPOSITORY)
    private readonly friendRequestRepository: FriendRequestRepository,
  ) {}

  execute(input: ListFriendRequestsInput): Promise<FriendRequest[]> {
    return this.friendRequestRepository.listRequests(input);
  }
}
